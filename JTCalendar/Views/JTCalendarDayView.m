...
- (void)commonInit
{
	...
	{
		_eventCircleView = [UIView new];
		[self addSubview:_eventCircleView];

		_eventCircleView.backgroundColor = [UIColor clearColor];
		_eventCircleView.hidden = YES;

		_eventCircleView.layer.rasterizationScale = [UIScreen mainScreen].scale;
		_eventCircleView.layer.shouldRasterize = YES;
	}
...
- (void)layoutSubviews
{
	...
	_eventCircleView.frame = CGRectMake(0, 0, sizeCircle, sizeCircle);
	_eventCircleView.center = CGPointMake(self.frame.size.width / 2., self.frame.size.height / 2.);

	[_eventCircleLayer setPath:[[UIBezierPath bezierPathWithOvalInRect:CGRectMake(0, 0, sizeCircle, sizeCircle)] CGPath]];
	[[_eventCircleView layer] addSublayer:_eventCircleLayer];
